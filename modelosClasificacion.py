...
class modelos():
    ...
    def modelLogisticRegression(self, parametrosLogReg):
        ...
        features_importance_LogReg.to_csv(self.path + '/' + self.nameProject + '/output/modelos/features_selection_LogisticRegression.csv', sep=';', index=False)
        createPlots().plotFeaturesSelect(features_importance_LogReg, self.path, self.nameProject, 'LogisticRegression')
        pd.DataFrame(model_lr.cv_results_).to_csv(self.path + '/' + self.nameProject + '/output/modelos/gridSearch_LogisticRegression.csv', index=False, sep=';')                
        scoreTrain = model_lr.score(X_train, y_train)
        scoreTest = model_lr.score(X_test, y_test)
        filename = self.path + '/' + self.nameProject + '/output/modelos/model_LogisticRegression.pkl'
    ...
    def modelRandomForest(self, parametrosRandomForest):    
        ...
        features_importance_RanFor.to_csv(self.path + '/' + self.nameProject + '/output/modelos/features_selection_RandomForest.csv', sep=';', index=False)
        createPlots().plotFeaturesSelect(features_importance_RanFor, self.path, self.nameProject, 'RandomForest')
        pd.DataFrame(model_rf.cv_results_).to_csv(self.path + '/' + self.nameProject + '/output/modelos/gridSearch_RandomForestClassifier.csv', index=False, sep=';')        
        scoreTrain = model_rf.score(X_train, y_train)
        scoreTest = model_rf.score(X_test, y_test)
        filename = self.path + '/' + self.nameProject + '/output/modelos/model_RandomForest.pkl'
    ...
    def modelGradientBoostingClassifier(self, parametrosGradBoosting):
        ...
        features_importance_GradBoosting.to_csv(self.path + '/' + self.nameProject + '/output/modelos/features_selection_GradBoosting.csv', sep=';', index=False)
        createPlots().plotFeaturesSelect(features_importance_GradBoosting, self.path, self.nameProject, 'GradBoosting')
        pd.DataFrame(model_GradBoosting.cv_results_).to_csv(self.path + '/' + self.nameProject + '/output/modelos/gridSearch_GradientBoostingClassifier.csv', index=False, sep=';')
        scoreTrain = model_GradBoosting.score(X_train, y_train)
        scoreTest = model_GradBoosting.score(X_test, y_test)
        filename = self.path + '/' + self.nameProject + '/output/modelos/model_GradientBoostingClassifier.pkl'
    ...
    def modelXGBClassifier(self, parametrosxgb):
        ...
        features_importance_xgb.to_csv(self.path + '/' + self.nameProject + '/output/modelos/features_selection_XGBClassifier.csv', sep=';', index=False)
        createPlots().plotFeaturesSelect(features_importance_xgb, self.path, self.nameProject, 'XGBClassifier')
        pd.DataFrame(model_xgb.cv_results_).to_csv(self.path + '/' + self.nameProject + '/output/modelos/gridSearch_XGBClassifier.csv', index=False, sep=';')
        scoreTrain = model_xgb.score(X_train, y_train)
        scoreTest = model_xgb.score(X_test, y_test)
        filename = self.path + '/' + self.nameProject + '/output/modelos/model_XGBClassifier.pkl'
    ...
    def modelLightGBMClassifier(self, parametroslgb):
        ...
        features_importance_lgb = pd.DataFrame(data={'Variables': X_train.columns, 'coef': model_lgb.best_estimator_.feature_importances_}, columns=['Variables', 'coef']).sort_values('coef', ascending=False).reset_index(drop=True)
        features_importance_lgb.to_csv(self.path + '/' + self.nameProject + '/output/modelos/features_selection_lgbClassifier.csv', sep=';', index=False)
        createPlots().plotFeaturesSelect(features_importance_lgb, self.path, self.nameProject, 'LGBMClassifier')
        pd.DataFrame(model_lgb.cv_results_).to_csv(self.path + '/' + self.nameProject + '/output/modelos/gridSearch_LGBMClassifier.csv', index=False, sep=';')
        scoreTrain = model_lgb.score(X_train, y_train)
        scoreTest = model_lgb.score(X_test, y_test)
        filename = self.path + '/' + self.nameProject + '/output/modelos/model_LGBMClassifier.pkl'
